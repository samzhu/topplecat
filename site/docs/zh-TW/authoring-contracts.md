...
它使用本機發布的 0.2.2 release-line artifact（Maven Central 發布是另外的維護者工作），並提供五項完全合成的保障課程；每一項完成後都會留下該課程的本機
...
Review 接受 repository-relative 的 `.md` 路徑。每條選定的 AC 都必須有精確、獨立一行的
帶 ID marker，例如 `<!-- topplecat:acceptance:AC-CHECKOUT-001 -->`。marker 是唯一的
機器 identity 與插入點；標題層級與文字只負責可讀性。一般文字或 fenced code 中的引用不會
選取範圍。重複、格式錯誤、舊 generic 或容器內 directive 會產生可修復的
`TC-SPEC-AC-*` 診斷。ToppleCat 不會讀取或翻譯 Gherkin `.feature` 檔案。
