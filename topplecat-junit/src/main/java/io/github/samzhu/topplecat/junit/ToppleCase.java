...
public final class ToppleCase {
  ...
  public ToppleCase(ToppleCaseData data) {
    ...
    data.expected()
        .propertyNames()
        .forEach(key -> consumption.put(key, ExpectedConsumption.UNTOUCHED));
  }
  ...
  private void markRead(String key) {
    ...
    consumption.computeIfPresent(
        key,
        (ignored, state) ->
            state == ExpectedConsumption.ASSERTED ? state : ExpectedConsumption.READ);
  }

  private ToppleCatException conversionFailure(
      String side, String key, String type, RuntimeException exception) {
