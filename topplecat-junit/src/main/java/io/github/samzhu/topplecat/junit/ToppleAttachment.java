...
/** A small allowlisted, step-scoped attachment request. Visibility is inherited from the current case. */
public final class ToppleAttachment {
    ...
    private ToppleAttachment(String title, String mediaType, String extension, byte[] content) {
        ...
        if (this.content.length > MAX_BYTES) {
            throw new IllegalArgumentException("Topple attachment " + title + " exceeds the 10 MiB per-file limit.");
        }
    }
    ...
    private static ToppleAttachment textLike(String title, String mediaType, String extension, String value,
                                             UnaryOperator<String> redactor) {
        String redacted = Objects.requireNonNull(redactor, "redactor").apply(Objects.requireNonNull(value, "value"));
        return new ToppleAttachment(title, mediaType, extension, redacted.getBytes(StandardCharsets.UTF_8));
    }
