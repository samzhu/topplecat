...
published_junit_jar="$root/topplecat-junit/build/libs/topplecat-junit-0.0.14.jar"
...
assert_artifact_version "$junit_sample/build.gradle.kts" "0.0.14"
assert_artifact_version "$spring_sample/build.gradle.kts" "0.0.14"
assert_artifact_version "$root/integration-tests/mutation-gate/build.gradle.kts" "0.0.14"
...
for verification_report in \
  ...
  if [[ ! -f "$verification_report" ]] \
    || [[ ! -f "$verification_data" ]] \
    || ! grep -Fq 'topplecat.verification-view.v9' "$verification_data"; then
