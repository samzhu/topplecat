...
final class EscrowUpdateAuditJson {
  ...
  static String write(EscrowUpdateAudit audit) {
    Payload payload =
        new Payload(
            audit.schemaVersion(),
            audit.updatedAt().toString(),
            audit.previousManifestSha256(),
            audit.newManifestSha256(),
            audit.previousApprovalDigest(),
            audit.newApprovalDigest(),
            audit.added(),
            audit.changed(),
            audit.removed());
  ...
  static EscrowUpdateAudit read(String source) {
    ...
    return new EscrowUpdateAudit(
        ...
        payload.previousManifestSha256(),
        payload.newManifestSha256(),
        payload.previousApprovalDigest(),
        payload.newApprovalDigest(),
        payload.added(),
        payload.changed(),
        payload.removed());
  }
