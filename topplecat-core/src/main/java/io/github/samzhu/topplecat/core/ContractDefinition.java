 ...
 *
 * <p>The definition deliberately has no execution status, duration, failure, or attachment fields.
 * Those belong to a {@link VerificationRun} and are projected separately.
 */
public record ContractDefinition(
    String schemaVersion, String digest, List<AcceptanceContract> acceptanceConditions) {
  public static final String SCHEMA_VERSION = "topplecat.contract-definition.v3";
  ...
  public static ContractDefinition withComputedDigest(
  ...
  private static List<AcceptanceContract> sorted(List<AcceptanceContract> values) {
    ...
    List<AcceptanceContract> result =
        values.stream().sorted(Comparator.comparing(AcceptanceContract::acId)).toList();
