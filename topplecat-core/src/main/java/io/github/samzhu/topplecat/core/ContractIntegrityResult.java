...
public record ContractIntegrityResult(
  ...
  public ContractIntegrityResult {
    if (!SCHEMA_VERSION.equals(schemaVersion)
        ...
        || !sortedPaths(addedPaths)
        || !sortedPaths(changedPaths)
        || !sortedPaths(removedPaths)
        || !sortedFields(changedPolicyFields)
        || !optionalDigest(approvedApprovalDigest)
    ...
    changedPolicyFields =
        List.copyOf(changedPolicyFields == null ? List.of() : changedPolicyFields);
  }
