...
public record StepTemplate(
  ...
  public StepTemplate(
      ...
      List<ArgumentBinding> argumentBindings,
      SourceRef sourceRef) {
    this(stepId, phase, tokens, argumentBindings, sourceRef, "");
  }
  ...
  public StepTemplate {
    ...
    stageBinaryName = stageBinaryName == null ? "" : stageBinaryName;
      ...
      try {
        ...
        if (index < 0 || index >= argumentBindings.size()) {
          throw new ToppleCatException(
              "Step template " + stepId + " references missing argument {" + token.value() + "}.");
        }
      } catch (NumberFormatException exception) {
        throw new ToppleCatException(
            "Step template " + stepId + " has a non-numeric argument token.", exception);
      }
    }
  }
}
