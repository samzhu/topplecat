...
/** Assigns PIT mutants to ACs from the exact canonical test methods that cover each mutant. */
public final class PitMutationAttributor {
    private PitMutationAttributor() {
    }

    /**
     * Evaluates every AC using PIT's full mutation matrix.
     * Each mutant belongs to every covering JUnit invocation whose class and method signature
     * match the compiled canonical contract. Class membership alone is deliberately insufficient:
     * one Java acceptance-test class may contain several canonical AC methods.
     */
    public static List<PitMutationAssessment> assess(
        ...
        if (report == null || !report.coverageMatrix()) {
            throw new ToppleCatException("PIT fullMutationMatrix=true is required for automatic per-AC attribution.");
        }
        ...
        for (String acId : canonicalMethodsByAc.keySet().stream().sorted().toList()) {
            List<CanonicalTestMethod> methods = canonicalMethodsByAc.get(acId).stream()
                    .map(CanonicalTestMethod::parse).sorted(Comparator.comparing(CanonicalTestMethod::identity)).toList();
            List<PitMutation> selected = report.mutations().stream()
                    .filter(mutation -> mutation.coveringTests().stream()
                            .anyMatch(test -> methods.stream().anyMatch(method -> method.matches(test))))
                    .toList();
            int detected = (int) selected.stream().filter(PitMutation::killed).count();
            int score = selected.isEmpty() ? 0 : (detected * 100) / selected.size();
            EvidenceVerdict verdict = !selected.isEmpty() && score >= threshold ? EvidenceVerdict.PASS : EvidenceVerdict.FAIL;
            List<String> testClasses = methods.stream().map(CanonicalTestMethod::className).distinct().toList();
            assessments.add(new PitMutationAssessment(acId, testClasses, threshold,
                    selected.size(), detected, score, verdict));
        }
    ...
    private record CanonicalTestMethod(String identity, String className, String junitSignature) {
        private static CanonicalTestMethod parse(String identity) {
            ...
            if (methodSeparator < 1 || parametersStart <= methodSeparator + 1 || parametersEnd < parametersStart) {
            ...
            String parameters = descriptorParameters(identity.substring(parametersStart + 1, parametersEnd), identity);
        ...
        private boolean matches(String pitName) {
            ...
            if (!(normalized.equals(className) || normalized.startsWith(className + ".")
        ...
        private static ToppleCatException invalidIdentity(String identity) {
            return new ToppleCatException("Canonical test method identity is invalid for PIT attribution: " + identity);
        }
    }
}
