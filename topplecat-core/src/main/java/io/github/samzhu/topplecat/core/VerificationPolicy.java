...
public record VerificationPolicy(
    ...
    public VerificationPolicy {
        if (toppleCatVersion == null || toppleCatVersion.isBlank() || mutationThreshold < 0 || mutationThreshold > 100
        ...
        if (mutationProducerKind == MutationProducerKind.DEFAULT && mutationProducerTaskPath != null) {
            throw new ToppleCatException("The default mutation producer must not declare a custom task path.");
        }
