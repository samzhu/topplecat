...
public record SelectedSpecScope(
  ...
  public static SelectedSpecScope create(
      List<SelectedSpecDocument> specDocuments, List<String> acceptanceConditionIds) {
    List<SelectedSpecDocument> documents =
        new ArrayList<>(specDocuments == null ? List.of() : specDocuments);
  ...
  private static String digest(List<SelectedSpecDocument> documents, List<String> acIds) {
    return Hashing.sha256(
        JSON.writeValueAsBytes(new DigestPayload(SCHEMA_VERSION, documents, acIds)));
  }
