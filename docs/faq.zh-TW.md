...
人類仍然要負責把規則與案例寫完整。ToppleCat 不會找出人類漏寫的需求，
也不會替規格沒有定義的輸入決定答案。它負責讓代理收到的公開題目、審閱者
核准的可執行合約，以及最後驗證時執行的合約保持一致，並確保報告沒有改變
這份合約的原意。
...
ToppleCat 0.0.6 使用 PIT Gradle 外掛 1.19.0、PIT 1.25.5，以及 PIT 的
