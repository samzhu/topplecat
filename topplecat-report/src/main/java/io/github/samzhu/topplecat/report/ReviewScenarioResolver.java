...
final class ReviewScenarioResolver {
    ...
    static List<ReviewScenarioStep> resolve(List<StepTemplate> templates, JsonNode inputs, JsonNode expected) {
        ObjectNode root = new ObjectNode(new JsonNodeFactory());
        root.set("inputs", inputs);
        root.set("expected", expected);
        return templates.stream().map(template -> new ReviewScenarioStep(template.phase(),
                ScenarioTemplateRenderer.renderSentence(template,
                        template.argumentBindings().stream().map(binding -> display(root, binding)).toList())))
                .toList();
    }
